...
| `data/raw/public/` | Labelled hospital datasets used to train and score the model (MITDB, SVDB, LTAFDB, etc.). See `Docs/DATASETS.md`. |
...
| `Docs/PIPELINE_METHODS_AND_RESULTS.md` | **The most important one.** Stage-by-stage methods, every measured result, and where each number came from. |
| `Docs/AGENT_RULES.md` | **Before touching the classifier or any model file.** Every rule exists because it was broken once. |
| `Docs/DATASETS.md` | You need to re-download a dataset. |
...
- The heartbeat classifier is reliable for the two classes that matter most:
  Normal (F1 0.972) and Ventricular (F1 0.826, the dangerous class).
...
`README.md` and `Docs/PIPELINE_METHODS_AND_RESULTS.md` §6.1 to match.
...
`Docs/AGENT_RULES.md` first.** Every rule in it exists because it was broken once
at real cost.
