...
from ecg_pipeline.agent_bridge import beat_summary_pct  # noqa: E402
from ecg_pipeline.ecg_pipeline_core import DATA_RAW  # noqa: E402
...
def build_combined_manifest(vitalpatch_manifest: Path, prorhythm_manifest: Path,
    ...
    rows = _read_manifest(vitalpatch_manifest, "vitalpatch") + _read_manifest(prorhythm_manifest, "prorhythm")
...
def print_showcase(rows: list[dict], reports_root: Path, n: int = 3) -> None:
    ...
    for label, r in picks[:n]:
        ...
        for cls, info in report_json["beat_summary"].items():
            print(f"  {cls}: {info['count']} ({beat_summary_pct(info)}% of detected)")
