...
*Source: `docs/thesis/DATASETS.md`.*
...
**Production model, DS2 held-out, 45,881 beats, never re-tuned against this split.**
...
**Why accuracy is not reported as a success metric:** N is 40,711 of 45,881 beats
(88.7%). A model that only ever predicts "N" scores 88.7% accuracy while being
clinically useless. The overall 0.922 accuracy figure is therefore not evidence
of anything. This is codified as rule 4 in `docs/decisions/AGENT_RULES.md`.

**What is actually good here:** V — the class that matters most clinically — is
at F1 0.830 with 0.907 sensitivity. That is the number the system's clinical
value rests on.
...
`docs/thesis/PROJECT_OVERVIEW.md` §7 still quotes a 902-segment partial snapshot —
...
1. **S-class recall is 14%; F-class is unsolved (F1 0.005).** Four independent
