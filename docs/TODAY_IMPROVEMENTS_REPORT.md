...
| 6 | NEWS2 partial-coverage decision | undocumented, and based on an incorrect assumption | options documented, not decided | `docs/decisions/NEWS2_PARTIAL_COVERAGE_DECISION.md`, commit `a9dec88` |
...
| **2 — Re-run ECG-only batch** | **PASS** | 0 `PARSE_ERROR` rows confirmed, README.md and `docs/thesis/PIPELINE_METHODS_AND_RESULTS.md` §6.1 updated with real numbers, manifest itself not committed (gitignored per `.gitignore:3`, confirmed via `git check-ignore`). |
...
- **Beat classifier**: Normal F1 0.972, Ventricular F1 0.826 (unchanged this
  ...
  `docs/decisions/AGENT_RULES.md` first).
...
- **The NEWS2 partial-coverage escalation policy is undecided.** Options are
  written up in `docs/decisions/NEWS2_PARTIAL_COVERAGE_DECISION.md`; no option has been
