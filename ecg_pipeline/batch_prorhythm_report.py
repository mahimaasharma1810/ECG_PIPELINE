...
(agent_bridge.run_full_report) over a selected set of prorhythm SeNSiO CSVs,
...
prorhythm/ is not yet in Docs/DATASETS.md. It was inspected directly for
this run: same "SeNSiO" CSV layout already documented for data/raw/sense_io/
...
VitalPatch nominal rate) -- so the existing parse_sensio_ecg() parser is
reused as-is, no new parser needed.

Reuses agent_bridge.run_full_report(), parse_sensio_ecg(), and
...
from ecg_pipeline.ecg_pipeline_core import DATA_RAW, MODELS_DIR, parse_sensio_ecg
...
def run_batch(files: list[Path], classifier_path: Path, out_dir: Path, manifest_path: Path) -> None:
    ...
    for f in files:
        print(f"\n--- {f.name} ---")
        try:
            recording = parse_sensio_ecg(f)
        except Exception as e:
            rows.append({
                "patient_id": None, "segment_id": f.stem, "source": "sensio", "duration_s": None,
        ...
        try:
            ...
            rows.append({
                "patient_id": recording.patient_id, "segment_id": recording.segment_id, "source": "sensio",
                "duration_s": None,
                "n_beats_detected": None, "n_beats_analyzed": None, "quality_score": None,
                "sqi_window_rejection_rate": None, "assessable": False, "final_risk_level": None,
                "n_rhythm_findings": None, "medgemma_status": None,
...
def main(argv=None):
    ...
    parser.add_argument("--files", nargs="*", default=None,
                         help="specific filenames (within --prorhythm-root) to process; default: all ECG_*.csv "
                              ...
                              "see the timestamps; parse_sensio_ecg already detects the ECG_Filtered column "
