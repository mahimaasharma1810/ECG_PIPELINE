...
from ecg_inference.preprocess import parse_vitalpatch_ecg, parse_sensio_ecg, parse_wfdb_record, MODELS_DIR
...
def _load_segments(source: str, input_path: Path) -> list:
    ...
    if source == "sensio":
        return [parse_sensio_ecg(input_path)]
    if source == "wfdb":
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--input", type=Path, required=True, help="raw ECG file (VitalPatch/SeNSiO CSV, or a WFDB record path)")
    parser.add_argument("--output", type=Path, required=True, help="where to write the structured JSON report")
    parser.add_argument("--source", choices=["vitalpatch", "sensio", "wfdb"], default="vitalpatch",
    ...
    parser.add_argument("--classifier", type=Path, default=MODELS_DIR / "five_class_xgb.json",
                         help="pretrained classifier weights (default: ecg_inference/models/five_class_xgb.json)")
